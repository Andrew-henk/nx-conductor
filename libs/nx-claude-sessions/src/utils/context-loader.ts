...
export class LibraryContextLoader {
  ...
  async loadContext(library: string): Promise<LibraryContext> {
    logger.info(`Loading context for library: ${library}`)

    const [claudeFile, dependencies, sharedResources, sessionHistory, patterns] = await Promise.all([
      this.loadClaudeFile(library),
      this.getNxDependencies(library),
      this.getSharedResourcesContext(library),
      this.getCompressedSessionHistory(library),
      this.getLibraryPatterns(library)
    ])
